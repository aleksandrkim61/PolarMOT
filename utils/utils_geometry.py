...
from tracking.utils_tracks import correct_new_angle_and_diff, compute_angle_diff
...
def tracking_distance_2d_full(coords_0: np.ndarray, coords_1: np.ndarray) -> float:
    # TODO: optimize all of this with numpy stuff
    dist = tracking_distance_2d_dims(coords_0, coords_1)
    # _, diff = correct_new_angle_and_diff(coords_0[3], coords_1[3])
    diff = compute_angle_diff(coords_0[3], coords_1[3])
    # assert diff <= np.pi / 2, f"diff {diff}"
    # cos_dist = 1 - np.cos(diff)  # in [0, 1] since diff in [0, pi/2]
    # return dist * (1 + cos_dist)  # multiplier is in [1, 2]
    # return dist * (2 - np.cos(diff))  # multiplier is in [1, 2]
    cos_dist = np.cos(diff) - 1  # in [-2, 0]
    cos_dist -= 1  # in [-3, -1]
    cos_dist *= -1  # in [3, 1]
    return dist * cos_dist  # cos_dist=2 for pi/2 as before, and 3 for completely different orientations
