...
import os
from typing import Optional, List, Callable, Iterable, Dict, Mapping, Union, Tuple
...
class MOTFrameNuScenes(MOTFrame):
    def __init__(self, sequence, name: str, nusc: NuScenes, params: Mapping):
    ...
    def bbox_3d_annotations(self, world: bool) -> List[Bbox3d]:  # List[Box]
        bboxes = [self.bbox_3d_annotation(token, world) for token in sorted(self.frame["anns"])]
        utils.assign_ids_to_frame_detections(self.sequence.frame_tokens.index(self.name), bboxes)
        return [bbox for bbox in bboxes if bbox is not None]

    def bbox_3d_annotation(self, annotation_token: str, world: bool) -> Optional[Bbox3d]:  # Box
        ...
        gt_track_id = hash(annotation["instance_token"])
        ...
        if bbox_class in nu_classes.ALL_NUSCENES_CLASS_NAMES:
            return Bbox3d.from_nu_box_convert(bbox_nu, gt_track_id)
        else:
            return None
