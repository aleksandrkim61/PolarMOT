...
import os
from typing import Optional, List, Dict, Set, Any, Iterable, IO, Mapping
...
class MOTSequenceNuScenes(MOTSequence):
    def __init__(self, det_source: str, seg_source: str, split_dir: str, split: str,
                 ...
                 dataset_detections_3d: Dict[str, List[Bbox3d]], params: Mapping, work_dir: str):
        self.nusc = nusc
        self.scene = scene
        ...
        super().__init__(det_source, seg_source, split_dir, self.scene["name"], self.frame_tokens, params, work_dir)
    ...
    def get_frame(self, frame_token: str) -> MOTFrameNuScenes:
        frame = MOTFrameNuScenes(self, frame_token, self.nusc, self.params)
    ...
    def class_enums_to_track(self) -> List[DatasetClassEnum]:
        return [c for c in nu_classes.NuScenesClasses]
    ...
    def report_mot_results(self, frame_name: str, bboxes_to_report: Iterable[Bbox3d],
        ...
        reporting.add_results_to_submit(self.dataset_submission, frame_name, bboxes_to_report)

    def save_mot_results(self, mot_3d_file: IO, mot_2d_from_3d_file: Optional[IO] = None) -> None: pass
