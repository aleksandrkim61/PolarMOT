...
from typing import List, Optional, Iterable, Tuple, Dict, Mapping, Set, IO, Any, Union
...
class MOTFrame(ABC):
    def __init__(self, sequence: mot_sequence.MOTSequence, name: str, params: Mapping):
    ...
    def fuse_instances_and_save(self, params: Mapping, run_info: Dict, *, load: bool = True, save: bool = True):
        if load:
            ...
            if Path.is_dir(dir_to_load):
                ...
                self.fused_instances.sort(key=lambda x: x.instance_id)
                return
    ...
    def match_3d_2d_dets_with_iou(self, dets_3d: List[Bbox3d], dets_2d_multicam: Mapping[str, List[Detection2D]],
        ...
        if dets_3d and dets_2d_multicam:  # If anything is detected in 3D
            raise RuntimeError(f"Should not have 2D dets {len(dets_2d_multicam)}")
    ...
    def perform_tracking(self, run_info: Dict = defaultdict(int)):
        start_mot_ego = time.time()
        bboxes_to_report = self.sequence.mot.update(self.bboxes_3d_world, run_info)
    ...
    def load_segmentations_2d_if_needed(self):
        if not self._dets_2d_multicam:
            ...
            self._dets_2d_multicam = defaultdict(list)
    ...
    @abstractmethod
    def bbox_3d_annotations(self, world: bool) -> List[Bbox3d]:
        pass
