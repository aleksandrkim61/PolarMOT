...
from typing import Optional, Dict, List, Set, Iterable, Sequence, IO, Tuple, Callable, Mapping, Union
...
class MOTFrameKITTI(mot_frame.MOTFrame):
    def __init__(self, sequence, name, params: Mapping):
        super().__init__(sequence, name, params)
    ...
    def transform_objects_to_world_frame(self, objects: Union[Iterable[FusedInstance],
                                                              Iterable[Bbox3d]]
                                         ) -> Tuple[Optional[np.ndarray], Optional[float]]:
        for obj in objects:
            obj.transform(self.ego_transform, self.angle_around_vertical)
    ...
    def bboxes_3d_world(self):
        bboxes = deepcopy(self.bboxes_3d)
        self.transform_objects_to_world_frame(bboxes)
        return bboxes

    def bbox_3d_annotations(self, world: bool) -> List[Bbox3d]:  # List[Box]
        bboxes = deepcopy(self.sequence.bbox_3d_annotations[self.name])
        if world:
            self.transform_objects_to_world_frame(bboxes)
        return bboxes
...
class MOTSequenceKITTI(mot_sequence.MOTSequence):
    def __init__(self, det_source: str, seg_source: str, split_dir: str, split: str,
                 name: str, frame_names: List[str], params: Mapping, work_dir: str):
        super().__init__(det_source, seg_source, split_dir, name, frame_names, params, work_dir)
    ...
    def get_frame(self, frame_name: str):
        frame = MOTFrameKITTI(self, frame_name, self.params)
    ...
    def load_detections_3d(self) -> Dict[str, List[Bbox3d]]:
        bboxes_3d_all: Dict[str, List[Bbox3d]] = loading.load_detections_3d(self.det_source, self.name)
    ...
    def bbox_3d_annotations(self) -> Dict[str, List[Bbox3d]]:
        # KITTI provides annotations in ego frame - return as is. Use frame.bbox_3d_annotations() to specify coordinate frame
        if not self._bbox_3d_annotations:
            self._bbox_3d_annotations = loading.load_annotations_kitti(self.name)
            self.assign_ids_to_detections(self._bbox_3d_annotations)
    ...
    def report_mot_results(self, frame_name: str, bboxes_to_report: Iterable[Bbox3d],
                           mot_3d_file: IO,
                           mot_2d_from_3d_only_file: Optional[IO] = None) -> None:
        reporting.write_to_mot_file(frame_name, bboxes_to_report, mot_3d_file, mot_2d_from_3d_only_file)

    def save_mot_results(self, mot_3d_file: IO, mot_2d_from_3d_file: Optional[IO] = None) -> None:
    ...
    def classes_to_track(self) -> List[int]:
        return kitti_classes.ALL_KITTI_CLASS_IDS

    @property
    def class_enums_to_track(self) -> List[DatasetClassEnum]:
        return [c for c in kitti_classes.KITTIClasses]
...
class MOTDatasetKITTI(mot_dataset.MOTDataset):
    ...
    def __init__(self, work_dir, det_source: str, seg_source: str, params: Mapping):
    ...
    def get_sequence(self, split: str, sequence_name: str) -> MOTSequenceKITTI:
        ...
        return MOTSequenceKITTI(self.det_source, self.seg_source, split_dir, split, sequence_name,
                                self.split_sequence_frame_names_map[split][sequence_name], self.params, self.work_dir)
